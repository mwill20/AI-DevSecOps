# 🎓 Lesson 01: The Scanner - Pattern Matching

## 🛡️ Welcome Back, AI-DevSecOps Analyst!

Ready to become a **code detective**? 🕵️‍♂️ Today we're diving into the **ScanEngine** - the "eyes" of our AI-DevSecOps system that spots bad patterns in BOTH human-written AND AI-generated code.
...
> "Did an AI agent generate this insecure pattern?" 🤖
...
## 📝 Code Walkthrough: The Pattern Hunter
...
export const OWASP_LLM_PATTERNS: SecurityPattern[] = [
  ...
  {
    id: 'LLM01-001',                    // 1️⃣ Unique identifier
    category: 'LLM01',                  // 2️⃣ OWASP category
    severity: 'HIGH',                   // 3️⃣ Risk level
    ...
    cweReference: 'CWE-74'             // 4️⃣ Industry standard reference
  },
...
1️⃣ **`id: 'LLM01-001'`** - Every pattern gets a unique ID like "LLM01-001". This helps us track exactly which rule was triggered.

2️⃣ **`category: 'LLM01'`** - This maps to the OWASP LLM Top 10 categories. LLM01 = Prompt Injection, LLM06 = Sensitive Data, etc.

3️⃣ **`severity: 'HIGH'`** - Risk levels: CRITICAL (secrets), HIGH (prompt injection), MEDIUM (insecure output), LOW (coding standards).

4️⃣ **`cweReference: 'CWE-74'`** - Links to industry-standard CWE (Common Weakness Enumeration) numbers for compliance.

### 🎯 The AI-DevSecOps Pattern Matching Magic
...
class PromptInjectionDetector implements ViolationDetector {
  ...
  async detect(content: string, context: ScanContext): Promise<SecurityViolation[]> {
    // 1️⃣ Get all LLM01 patterns
    const patterns = OWASP_LLM_PATTERNS.filter(p => p.category === 'LLM01');
    
    // 2️⃣ Test each pattern against the code
    for (const pattern of patterns) {
      const matches = content.matchAll(pattern.pattern);
      
      // 3️⃣ Create violation objects for each match
...
*Remember: Good security analysts understand the patterns before they enforce the rules!* 🛡️
